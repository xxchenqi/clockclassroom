 ...
 * 课室订单列表_碎片
 * <p/>
 * Created by wh on 2016/6/14
 */
public class FragmentOrderClassRoom extends BaseFragment
        implements MineOrderAdapter.OrderClickListener,
...
//                                getHttpUtils();
    ...
    protected void initView() {
        ...
        data = new ArrayList<>();
        if ("-1".equals(status)) {
            mineOrderAdapter = new MineOrderAdapter(getActivity(), data,
                    R.layout.item_mineorder, this, true);
        } else {
            mineOrderAdapter = new MineOrderAdapter(getActivity(), data,
...
//                }
    ...
    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
        if (cbFlag) {
            ...
            tv_total_price.setText(
                    String.format(
                            getString(R.string.how_much_money),
                            currentPrice + ""
                    ));
            mineOrderAdapter.notifyDataSetChanged();
        }
     ...
     * 支付宝支付
     */
    private void parseData(String oid, final CartCommit commitInfo, final String sid,
