...
public class CalendarPickerView extends ListView {
	...
	public void setOrangeDates(Set<Date> orangeDates) {
		this.orangeDates = orangeDates;
		if (null != adapter) {
			adapter.notifyDataSetChanged();
		}
	}
	...
	public CalendarPickerView(Context context, AttributeSet attrs) {
		...
		titleTextColor = a.getColor(
				...
				res.getColor(R.color.calendar_text_my));
		displayHeader = a.getBoolean(
				R.styleable.CalendarPickerView_tsquare_displayHeader, true);
	...
	private class CellClickedListener implements MonthView.Listener {
		...
		public void handleClick(MonthCellDescriptor cell) {
			...
			if (cellClickInterceptor != null
					&& cellClickInterceptor.onCellClicked(clickedDate)) {
				return;
			}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (monthView == null) {
				monthView = MonthView.create(parent, inflater,
						...
						displayHeader, headerTextColor, decorators, locale);
			} else {
				monthView.setDecorators(decorators);
			}
			if(SPposition != Integer.MAX_VALUE ){
//				Logr.d("第一次中转SPposition"+SPposition);
//				Logr.d("第一次中转EPposition"+EPposition);
				monthView.setSPposition(SPposition);
				monthView.setEPposition(EPposition);
				monthView.setSMPposition(SMPposition);
				monthView.setEMPposition(EMPposition);
				monthView.setSYPposition(SYPposition);
